...
mod tests {
  ...
  fn bumps_preminor_with_preid() {
    ...
    let update =
      update_manifest_version(manifest, &VersionRequest::Bump(Bump::Preminor), Some("rc")).unwrap();
  ...
  fn rejects_manifest_without_package_name() {
    ...
    let error =
      update_manifest_version(manifest, &VersionRequest::Bump(Bump::Patch), None).unwrap_err();
